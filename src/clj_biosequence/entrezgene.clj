...
(extend entrezInterval
  ...
  (assoc bs/default-biosequence-interval
    ...
    :comp?
    (fn [this]
      (= (bs/get-text this :Seq-interval_strand :Na-strand
                      (zf/attr :value))
         "minus"))))
...
(extend entrezGene
  ...
  (assoc bs/default-biosequence-translation
    :trans-table (fn [this]
                   (bs/get-text (org-name this) :OrgName_gcode)))
  bs/biosequenceID
  (assoc bs/default-biosequence-id
  ...
  (assoc bs/default-biosequence-taxonomies
    :tax-refs
    (fn [this]
      (list (entrez-biosource (org-ref this)))))
